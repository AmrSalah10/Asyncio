...
async def download_single_image(session: aiohttp.ClientSession, url: str, img_num: int) -> Path:
...
async def download_images(urls: list) -> list[Path]:
    async with aiohttp.ClientSession() as session:
        async with asyncio.TaskGroup() as tg:
            tasks = [
                tg.create_task(download_single_image(session, url, img_num))
...
async def process_images(orig_paths: list[Path]) -> list[Path]:
    ...
    with ProcessPoolExecutor() as executor:
        tasks = [loop.run_in_executor(executor, process_single_image, orig_path) for orig_path in orig_paths]
...
"""
Downloaded 12 images in: 4.22 seconds. 41.63% of total time
Processed 12 images in: 5.92 seconds. 58.37% of total time

Total execution time: 10.14 seconds. 100.00% of total time
"""